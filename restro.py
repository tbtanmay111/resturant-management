...
first=Label(frame1,text='OUR RESTARUANT',bg='green',font=('Comic Sans MS',30,'underline'))
...
def totalAmount():
    global sum
    totalout.config(text=sum)
    sgstv = sum * (sgst/100)
    cgstv = sgstv
    sum +=(sgstv*2)
    nettotalout.config(text = sum)
def billNo():
    global billNumscr
    global sgst
    global cgst
...
bill = Button(frame1, text='BILL',command= lambda:billNo(),height=2,width=20)
bill.grid(row=1, column=7)
...
total.grid(row=5, column=7)
totalout = Button(frame1, height=1, width=15, relief='sunken', borderwidth=5, bg="powder blue")
totalout.grid(row=5, column=8, padx=5, pady=5, ipadx=5, ipady=5)
